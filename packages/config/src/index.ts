...
const EnvSchema = z.object({
  ...
  TEMPORAL_TLS: z.string().default('true').transform((v) => v === 'true'),
  ...
  // BFF
  BFF_PORT: z.string().default('4000').transform((v) => Number(v)),
  ...
  APPROVAL_TOKEN_TTL_MINUTES: z.string().default('60').transform((v) => Number(v)),
  ...
  // Feature flags
  FEATURE_DRY_RUN_ENGINE: z.string().default('true').transform((v) => v === 'true'),
  FEATURE_EXECUTION_ENGINE: z.string().default('true').transform((v) => v === 'true'),
  FEATURE_LIVE_CONNECTORS: z.string().default('false').transform((v) => v === 'true'),
  FEATURE_KILL_SWITCH: z.string().default('true').transform((v) => v === 'true'),
});
